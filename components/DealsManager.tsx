...
export default function DealsManager({ initial }: { initial: Deal[] }) {
  ...
  // Apple-UX FLIP: when a deal changes stage, remaining cards FLOW to their new
  // positions instead of jumping. Additive, transform-only, reduced-motion aware.
  ...
  return (
    <div>
      ...
      <div ref={boardRef} className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          return (
            <div key={key} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3">
              ...
              <div className="space-y-2">
                {col.map((d) => (
                  <div key={d.id} data-flip-id={d.id} className="rounded-lg border border-[var(--border)] p-2">
